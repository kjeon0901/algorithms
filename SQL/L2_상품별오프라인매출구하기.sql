...
/*
INNER JOIN(=JOIN)에서는 일치하지 않는 레코드는 모두 버리지만, 
OUTER JOIN에서는 일치하지 않더라도 버리지 않고 NULL로 채워서 결과를 응답한다.

처음에는 이 문제를 FULL OUTER JOIN으로 해결하려고 했는데,
그럴 필요 없이 INNER JOIN으로 해결되는 문제였다. 
앞으로 이 둘을 잘 구분해서 풀어야겠다. 
*/
